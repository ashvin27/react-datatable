                ...
                <div className="row table-head asrt-table-head"
                    ...
                    <div className="col-md-6">
                        {(that.config.show_length_menu) ? (
                            <div className="input-group asrt-page-length">
                                ...
                                {(_.includes(that.config.language.length_menu, '_MENU_')) ? (
                                    <select type="text" className="form-control"
                                            style={style.table_size_dropdown}
                                            onChange={that.changePageSize.bind(that)}>
                                        {that.config.length_menu.map((value, key) => {
                                            return (<option key={value}>{value}</option>);
                                        })}
                                        <option value={that.props.records.length}>All</option>
                                    </select>
                                ) : null}
                                <div className="input-group-addon input-group-prepend">
                                          <span className="input-group-text" style={style.table_size}>
                    ...
                    <div className="col-md-6 float-right text-right">
                        ...
                        <div className="table_tools" style={style.table_tool}>
                            ...
                            {(that.config.button.extra==true) ? (

                                that.props.extraButtons.map((elem,index)=>{
                                    elem.clickCount=0;
    ...
    render() {
        ...
        return (
            <div className="as-react-table" id={(this.props.id) ? this.props.id + "-container" : ""}>
                {
                    this.showHeader(
                        this.config.show_length_menu == true ||
                        this.config.show_filter == true ||
                        this.config.button.excel == true ||
                        this.config.button.csv == true ||
                        this.config.button.print == true ||
                        this.config.button.extra == true
                    )
                }
                                            ...
                                            this.props.columns.map((column, colIndex) => {
                                                if (column.cell && typeof column.cell === "function") {
                                                    return (<td className={column.className}
                                                                key={(column.key) ? column.key : column.text}>{column.cell(record, rowIndex)}</td>);
                                                } else if (record[column.key]) {
                                                    return (<td className={column.className}
                                                                key={(column.key) ? column.key : column.text}>
                                                        {record[column.key]}
                                                    ...
                                                    return <td className={column.className}
                                                               key={(column.key) ? column.key : column.text}></td>
                                                }
                                            })
                                        }
                                    </tr>
